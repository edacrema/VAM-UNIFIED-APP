...
from app.services.mfi_drafter import router, execution, execution_service, drafts, light_service
from app.shared import async_runs


def test_previous_checkpoints_remain_inspectable_but_cannot_resume_under_light_bundle(monkeypatch):
    ...
    assert status["status"] == "failed" and not status["resumable"] and status["draft_available"]
    ...
    assert first.json() == repeated.json() and len(scheduled) == 0
...
def test_resume_storage_failure_returns_503(monkeypatch):
    ...
    monkeypatch.setattr(light_service,"recovery_store",unavailable)
    app = FastAPI()
    app.include_router(router.router,prefix="/mfi-drafter")
